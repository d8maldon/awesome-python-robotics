...
- **05 LQR triple-pendulum** — all 3 MAJORs HONOURED (controllability rank check 8/8, Coriolis Jacobian remark, ROA via Lyapunov sublevel-set sampling — certified bound z^T P z ≤ 0.272 vs V(z0) = 0.685 with honest discussion that sampling-based bounds are conservative and SOS programming gives tighter bounds) + 2 MINORs (Bryson's rule comment, np.linalg.solve regularization note).
